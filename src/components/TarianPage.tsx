...
import { tarianData, CulturalItem } from '../data/culturalData';

interface TarianPageProps {
  openModal?: (item: CulturalItem) => void;
}

const TarianPage: React.FC<TarianPageProps> = ({ openModal }) => {
  const [isVisible, setIsVisible] = useState(false);
  const [hoveredCard, setHoveredCard] = useState<string | null>(null);
  const items = tarianData;
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 py-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-lg text-gray-700 max-w-md mx-auto">
            Maaf, tidak ada item tarian saat ini.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 py-16 px-4 relative overflow-hidden">
      {/* Decorative Background Pattern */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none">
        <div className="w-full h-full bg-repeat" style={{
          backgroundImage: `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%238B4513' fill-opacity='0.1'%3E%3Cpath d='M50 50c0-5.5-4.5-10-10-10s-10 4.5-10 10 4.5 10 10 10 10-4.5 10-10zm20-20c0-5.5-4.5-10-10-10s-10 4.5-10 10 4.5 10 10 10 10-4.5 10-10zM30 70c0-5.5-4.5-10-10-10s-10 4.5-10 10 4.5 10 10 10 10-4.5 10-10z'/%3E%3C/g%3E%3C/svg%3E")`
        }} />
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-100 to-orange-100 border border-amber-200 rounded-full px-6 py-2 mb-8">
            <span className="text-2xl">💃</span>
            <span className="text-amber-800 font-medium">Koleksi Tarian</span>
          </div>
          ...
          <p className="text-lg sm:text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Temukan {items.length} tarian indah yang mencerminkan kebudayaan dan ekspresi seni Jawa Timur
          </p>
        </div>
